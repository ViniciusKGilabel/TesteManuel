...
// CompletePayment updates the payment row to COMPLETED and the idempotency key to COMPLETED
// in a single transaction. If either write fails the transaction is rolled back and
// an error is returned — the caller must not publish payment.processed in that case.
func (u *UnitOfWork) CompletePayment(ctx context.Context, p *payment.Payment, key string) error {
	...
	`, string(payment.IdempotencyCompleted), key); err != nil {
